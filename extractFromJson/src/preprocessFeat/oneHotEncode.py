...
def onehotBegin():
    ...
    output_file_name = cfg.get("filename", "onehotOutput")
    fieldName = cfg.get("onehotfield", "onehotfieldname")
    with open(result_dir+output_file_name,"w",newline='') as f:
        all_possible_value,encodedvalue = onehotEncode(result_dir+input_file_name,fieldName)
        writer = csv.writer(f)
